 ...
 */
	...
	var initial_difference_pass = 0;
	//var willpoint = 524392 # 131098 points in map
			...
			for(var j=0 ; j< 2 ; j++){
				for(var i=0 ; i< env.frameRate/120 ; i++){	
					...
					if (env.time > 25000 && env.time < 25000 + 2.0*env.dt){
						...
						saveCsvFile(willDataForOnePoint,env.tau_pw,env.tau_d,'jacobian_x1')
...
function saveCsvFile(willDataForOnePoint,tau_pw,tau_d,lya_type){
	...
	*/
	...
	var csv = encodeURI(  willDataForOnePoint ) ;
	link.setAttribute( 'href', csv ) ;
	
	var name_download = 'tau_pw_' + tau_pw +  '_tau_d_' + tau_d + lya_type + '_APD_step_' + (2*env.skip*env.dt) + '.csv'
